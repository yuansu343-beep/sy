...
class MergedAdaBelief(Optimizer):
    ...
    参数说明：
        ...
        decay_after_update: 是否在更新后进行权重衰减
    """
    ...
    def __init__(
        ...
        defaults = dict(
            ...
            p_adapt=p_adapt,
            amsgrad=amsgrad,
            decay_after_update=decay_after_update,
        )
    ...
    def __setstate__(self, state):
        ...
        for group in self.param_groups:
            ...
            group.setdefault("decay_after_update", False)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # -------------------------------------------------
                # 1) 权重衰减 (decay before update)
                # -------------------------------------------------
                if weight_decay != 0 and not decay_after_update:
                    if weight_decouple:
                        # AdamW风格：解耦权重衰减
                ...
                # -------------------------------------------------
                # 9) 权重衰减 (decay after update)
                # -------------------------------------------------
                if weight_decay != 0 and decay_after_update:
                    if weight_decouple:
                        # AdamW风格：解耦权重衰减
                        p.mul_(1.0 - lr * weight_decay)
                    else:
                        # 耦合式L2正则 - 注意：这种情况下decay after update效果不同
                        # 这里我们仍然使用与decay before相同的方式处理
                        p.mul_(1.0 - lr * weight_decay)
